    ...
    switch (formName) {
        ...
        case 'set!':
            ...
            if (this.isLambda(exp)) {
                value = this.makeProcedure(this.lambdaParameters(exp), this.lambdaBody(exp), scope);
            } else {
        ...
        case 'lambda':
            return this.makeProcedure(this.lambdaParameters(expression), this.lambdaBody(expression), scope);
...
Runtime.prototype.extendEnvironment = function(params, args, env) {
    if (params.name !== undefined && args.length === 1) {
        return new Scope([params.name], args, env);
    }

    return new Scope(params.map(function(p) { return p.name ? p.name : p; }), args, env);
}

Runtime.prototype.apply = function(procedure, args) {
    if (typeof procedure === "function") {
        return procedure.apply(procedure.scope, args);
